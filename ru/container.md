...
## Распознавание нулевой конфигурации
Если класс не имеет зависимости или зависит от другого конкретного класса (не интерфейсов), контейнеру не нужно указывать, как распознавать этот класс. Для примера, вы можете разместить следующий код в файле `routes/web.php`:
...
```
В данном примере, при переходе по маршруту `/`, приложение автоматически распознает класс `Service` и внедрит его в обработчик маршрута.
